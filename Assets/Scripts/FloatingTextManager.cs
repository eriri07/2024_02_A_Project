...
public class FloatingTextManager : MonoBehaviour
{
    ...
    {
        Vector2 screenPos = Camera.main.WorldToScreenPoint(worldPos);       //월드 좌표를 스크린 좌표로 전환

        GameObject textOjb = Instantiate(textPrefabs, transform);           //UI 텍스트 생성
        ...
        if (temp != null)
        {
            ...
            StartCoroutine(AnimateText(textOjb));                           //만든 애니메이션 효과 재생
        }
    }
    ...
    private IEnumerator AnimateText(GameObject textObj)
    {
        ...
        while (timer < duration)                                            //타이머 1초 전까지
        {
            ...
            textObj.transform.position = startPost + Vector3.up * (progress * 50f);     //폰트를 위로 올라가는 효과를 준다
